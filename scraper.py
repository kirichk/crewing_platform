...
def vacancies_search(pages: list):
    ...
    for page in pages:
        ...
        for rows in table.find_all('tr')[1:]:
            columns = ['Position','Fleet','Vessel type',
                        'Salary','Joining date','Vacancy posted']
            info = []
...
def info_search(vacancies: dict, mode: str):
    ...
    for vacancy in vacancies:
        sleep(0.5)
        soup = BeautifulSoup(
            requests.get(vacancy['link']).text,
            'lxml')
        ...
        other_info = re.sub(r'\n+', '\n', details.text).strip()
        additional_info = other_info.replace('Дополнительная информация:\n','')
        print(f"Ссылка: {vacancy['link']}\n"\
                f"Позиция: {vacancy['Position']}\n"\
                f"Флот: {vacancy['Fleet']}\n"\
                f"Тип судна: {vacancy['Vessel type']}\n"\
                f"Зарплата: {vacancy['Salary']}\n"\
                f"Крюинг: {crewer}\n"\
                f"Дата посадки: {vacancy['Joining date']}\n"\
                f"Дата публикации: {vacancy['Joining date']}\n"\
                f"Длительность рейса: {voyage_duration}\n"\
                f"Регион работы: {sailing_area}\n"\
                f"DWT: {dwt}\n"\
                f"Год постройки судна: {years_constructed}\n"\
                f"Экипаж: {crew}\n"\
                f"Уровень английского: {english}\n"\
                f"Доп инфа: {additional_info}\n"\
                )
        updated_date = '20'+ vacancy['Joining date'][6:] + "-" \
                        + vacancy['Joining date'][3:5] + "-" \
                        + vacancy['Joining date'][:2]
        new_post = Post.objects.get_or_create(title = vacancy['Position'],
                                    fleet = vacancy['Fleet'],
